...
"""
    ...
    def predict(self, message: str, history: List[Tuple[str, str]]) -> str:
        """
        Генерирует ответ на запрос пользователя с учетом контекста и истории.
        ...
        try:
            self.context = self.get_rag_prompt_ready(
                message,
            ...
            if self.memory_size <= self.max_memory_size and (len(self.context) + self.memory_size) <= self.max_prompt_size:
